...
export default function PokerIndexPage() {
  ...
  const goToRoom = (roomId: number, roomCode: string, nick: string) => {
    const url = `/pages/poker/room/index?roomId=${roomId}&roomCode=${roomCode}&nickname=${encodeURIComponent(nick)}`;
    // 保存最近房间到本地
  ...
  const handleCreateRoom = async () => {
    ...
    setLoading(true);
    try {
      const res = await createRoom(userId, maxSeats, nickname.trim());
      if (res.code === 200 && res.data) {
        const room = res.data;
  ...
  return (
    <View className="index-page">
      <View className="header">
        <Text className="title">🃏 最爱的十三钗</Text>
        <Text className="subtitle">小华华的酒桌小游戏</Text>
      </View>
      ...
      <View className="section">
        <View className="section-header">
          <Text className="section-icon">🏗️</Text>
          <Text className="section-title">创建新房间</Text>
        </View>
        <Text className="section-desc">创建房间，自动成为房主</Text>
      ...
      <View className="section">
        ...
        <Text className="section-desc">输入6位房间号，加入好友创建的房间</Text>
        <Input
